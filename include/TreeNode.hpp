...
public:
    ...
    virtual Key get_key(){return DELETE_MARKER;};
    virtual void push_key(Key key){};
    virtual void del_key(){};

    //key functions
    virtual TreePtr insert_key(const Key& key, const RecordPtr& record_ptr) {return NULL_PTR;}
    virtual void delete_key(const Key& key) {}
    virtual void merge_siblings(const TreePtr &tree_ptr) {};
    virtual void redistribute_siblings(const TreePtr &tree_ptr, int check) {};
